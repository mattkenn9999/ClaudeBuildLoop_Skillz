...
- **Match the approach to the depth of the change.** Most changes are **additive** (new fields, new features on top of an unchanged core): preserve the existing features untouched and append, as above. Some changes are a **replacement** (they swap out a core mechanic, so most existing features change anyway). For a replacement, it is acceptable to regenerate the feature list cleanly with everything at `passes: false`, rather than forcing surgical preservation of a handful of survivors, because the code genuinely has to be re-verified against the new behaviour. Regenerating the list does **not** mean rebuilding the app: the build step still edits the existing code incrementally. What you must not do is the dishonest middle path of leaving features green that no longer reflect the design.
...
- **Always report the scope of the change, even if you regenerated the list.** Before handing off, tell the user plainly which behaviours you **preserved**, which you **revised** (and why), and which you **added**. This report is required whether you appended to the existing list or regenerated it: a renumbered list of all-`false` features with no explanation hides what actually changed, which is exactly the visibility a reviewer needs before the loop runs.
